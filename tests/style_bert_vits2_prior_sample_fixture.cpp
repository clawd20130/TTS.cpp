...
bool compare_vectors(const std::vector<float> & actual, const std::vector<float> & expected, double tolerance) {
    ...
    std::fprintf(stdout,
                 "STYLE_BERT_VITS2_PRIOR_SAMPLE_FIXTURE max_abs=%g rms=%g worst=%zu actual=%f expected=%f samples=%zu\n",
                 max_abs,
                 rms,
                 worst,
                 actual[worst],
...
int main() {
    ...
    if (!std::filesystem::is_regular_file(model_path)) {
        std::fprintf(stderr, "Style-Bert GGUF missing; skipping prior sample fixture test: %s\n", model_path.c_str());
        return 0;
    }

    const std::filesystem::path dir = fixture_dir();
    const std::filesystem::path m_p_path = dir / "akai_chi_frontend.m_p_expanded.f32";
    const std::filesystem::path logs_p_path = dir / "akai_chi_frontend.logs_p_expanded.f32";
    const std::filesystem::path expected_path = dir / "akai_chi_frontend.z_p.f32";
    if (!std::filesystem::is_regular_file(m_p_path) ||
        !std::filesystem::is_regular_file(logs_p_path) ||
        !std::filesystem::is_regular_file(expected_path)) {
        std::fprintf(stderr, "Style-Bert front fixture raw files missing; skipping prior sample fixture test in %s\n", dir.c_str());
        return 0;
    }
    ...
    std::vector<float> m_p = read_f32_file(m_p_path);
    std::vector<float> logs_p = read_f32_file(logs_p_path);
    std::vector<float> expected = read_f32_file(expected_path);
    if (m_p.size() != logs_p.size() || m_p.size() != expected.size()) {
        std::fprintf(stderr, "prior fixture size mismatch: m_p=%zu logs_p=%zu z_p=%zu\n",
                     m_p.size(), logs_p.size(), expected.size());
        return 1;
    }
    if (m_p.size() % runner->model->inter_channels != 0) {
        std::fprintf(stderr, "invalid prior fixture size %zu for inter_channels=%u\n",
                     m_p.size(), runner->model->inter_channels);
        return 1;
    }

    constexpr float noise_scale = 0.6f;
    std::vector<float> noise(m_p.size());
    for (size_t i = 0; i < m_p.size(); ++i) {
        noise[i] = (expected[i] - m_p[i]) / (std::exp(logs_p[i]) * noise_scale);
    }

    const uint32_t frames = (uint32_t) (m_p.size() / runner->model->inter_channels);
    std::vector<float> actual;
    runner->sample_prior(m_p.data(), logs_p.data(), noise.data(), frames, noise_scale, actual);
