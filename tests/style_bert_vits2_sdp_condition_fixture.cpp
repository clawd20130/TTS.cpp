...
bool compare_vectors(const std::vector<float> & actual, const std::vector<float> & expected, double tolerance) {
    ...
    std::fprintf(stdout,
                 "STYLE_BERT_VITS2_SDP_CONDITION_FIXTURE max_abs=%g rms=%g worst=%zu actual=%f expected=%f samples=%zu\n",
                 max_abs,
                 rms,
                 worst,
                 actual[worst],
...
int main() {
    ...
    if (!std::filesystem::is_regular_file(model_path)) {
        std::fprintf(stderr, "Style-Bert GGUF missing; skipping SDP condition fixture test: %s\n", model_path.c_str());
        return 0;
    }

    const std::filesystem::path dir = fixture_dir();
    const std::filesystem::path x_path = dir / "akai_chi_frontend.x.f32";
    ...
    const std::filesystem::path expected_path = dir / "akai_chi_frontend.sdp_condition.f32";
    if (!std::filesystem::is_regular_file(x_path) ||
        ...
        std::fprintf(stderr, "Style-Bert front fixture raw files missing; skipping SDP condition fixture test in %s\n", dir.c_str());
        return 0;
    }
    ...
    const uint32_t tokens = (uint32_t) x_mask.size();
    if ((size_t) tokens * runner->model->hidden_channels != x.size() ||
        (size_t) tokens * runner->model->hidden_channels != expected.size()) {
        std::fprintf(stderr, "invalid x/expected size: x=%zu expected=%zu tokens=%u hidden_channels=%u\n",
                     x.size(),
                     expected.size(),
                     tokens,
    ...
    runner->run_stochastic_duration_condition(x.data(), x_mask.data(), g.data(), tokens, actual);
    // PyTorch F.gelu defaults to the exact erf formulation, while ggml_gelu uses
    // the standard tanh approximation. The remaining error is bounded by that
    // activation difference after three DDSConv layers.
    if (!compare_vectors(actual, expected, 5e-4)) {
        return 1;
    }
    return 0;
}
