...
bool compare_vectors(const std::vector<float> & actual, const std::vector<float> & expected, double tolerance) {
    ...
    std::fprintf(stdout,
                 "STYLE_BERT_VITS2_DURATION_PREDICTOR_FIXTURE max_abs=%g rms=%g worst=%zu actual=%f expected=%f samples=%zu\n",
                 max_abs,
                 rms,
                 worst,
                 actual[worst],
...
int main() {
    ...
    if (!std::filesystem::is_regular_file(model_path)) {
        std::fprintf(stderr, "Style-Bert GGUF missing; skipping duration predictor fixture test: %s\n", model_path.c_str());
        return 0;
    }
    ...
    const std::filesystem::path expected_path = dir / "akai_chi_frontend.logw_dp.f32";
    if (!std::filesystem::is_regular_file(x_path) ||
        ...
        std::fprintf(stderr, "Style-Bert front fixture raw files missing; skipping duration predictor fixture test in %s\n", dir.c_str());
        return 0;
    }
    ...
    const uint32_t tokens = (uint32_t) expected.size();
    ...
    runner->predict_duration(x.data(), x_mask.data(), g.data(), tokens, actual);
