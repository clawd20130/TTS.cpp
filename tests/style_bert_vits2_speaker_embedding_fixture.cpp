...
int32_t read_i64_scalar_as_i32(const std::filesystem::path & path) {
...
bool compare_vectors(const std::vector<float> & actual, const std::vector<float> & expected, double tolerance) {
    ...
    std::fprintf(stdout,
                 "STYLE_BERT_VITS2_SPEAKER_EMBEDDING_FIXTURE max_abs=%g rms=%g worst=%zu actual=%f expected=%f samples=%zu\n",
                 max_abs,
                 rms,
                 worst,
                 actual[worst],
...
int main() {
    ...
    if (!std::filesystem::is_regular_file(model_path)) {
        std::fprintf(stderr, "Style-Bert GGUF missing; skipping speaker fixture test: %s\n", model_path.c_str());
        return 0;
    }

    const std::filesystem::path dir = fixture_dir();
    const std::filesystem::path speaker_path = dir / "akai_chi_frontend.speaker_id.i64";
    const std::filesystem::path expected_path = dir / "akai_chi_frontend.g.f32";
    if (!std::filesystem::is_regular_file(speaker_path) ||
        !std::filesystem::is_regular_file(expected_path)) {
        std::fprintf(stderr, "Style-Bert speaker fixture raw files missing; skipping fixture test in %s\n", dir.c_str());
        return 0;
    }
    ...
    const int32_t speaker_id = read_i64_scalar_as_i32(speaker_path);
    std::vector<float> expected = read_f32_file(expected_path);
    if (expected.size() != runner->model->gin_channels) {
        std::fprintf(stderr, "invalid g size %zu for gin_channels=%u\n", expected.size(), runner->model->gin_channels);
        return 1;
    }

    std::vector<float> actual;
    runner->encode_speaker(speaker_id, actual);
