...
bool compare_vectors(const std::vector<float> & actual, const std::vector<float> & expected, double tolerance) {
    ...
    std::fprintf(stdout,
                 "STYLE_BERT_VITS2_TEXT_ENCODER_LAYER_FIXTURE max_abs=%g rms=%g worst=%zu actual=%f expected=%f samples=%zu\n",
                 max_abs,
                 rms,
                 worst,
                 actual[worst],
...
int main() {
    ...
    if (!std::filesystem::is_regular_file(model_path)) {
        std::fprintf(stderr, "Style-Bert GGUF missing; skipping TextEncoder layer fixture test: %s\n", model_path.c_str());
        return 0;
    }
    ...
    const std::filesystem::path expected_path = dir / "akai_chi_frontend.encoder_layer0_norm2.f32";
    if (!std::filesystem::is_regular_file(x_path) ||
        ...
        std::fprintf(stderr, "Style-Bert TextEncoder layer fixture raw files missing; skipping fixture test in %s\n", dir.c_str());
        return 0;
    }
    ...
    runner->run_text_encoder_layer(0, x.data(), x_mask.data(), g.data(), tokens, actual);
