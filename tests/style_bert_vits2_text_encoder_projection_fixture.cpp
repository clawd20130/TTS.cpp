...
bool compare_vectors(const char * label, const std::vector<float> & actual, const std::vector<float> & expected, double tolerance) {
    ...
    std::fprintf(stdout,
                 "STYLE_BERT_VITS2_TEXT_ENCODER_PROJECTION_FIXTURE %s max_abs=%g rms=%g worst=%zu actual=%f expected=%f samples=%zu\n",
                 label,
                 max_abs,
                 rms,
                 worst,
...
int main() {
    ...
    if (!std::filesystem::is_regular_file(model_path)) {
        std::fprintf(stderr, "Style-Bert GGUF missing; skipping text encoder projection fixture test: %s\n", model_path.c_str());
        return 0;
    }
    ...
    if (!std::filesystem::is_regular_file(x_path) ||
        ...
        std::fprintf(stderr, "Style-Bert front fixture raw files missing; skipping text encoder projection fixture test in %s\n", dir.c_str());
        return 0;
    }
    ...
    std::vector<float> expected_m = read_f32_file(m_path);
    std::vector<float> expected_logs = read_f32_file(logs_path);
    const uint32_t tokens = (uint32_t) x_mask.size();
    if ((size_t) tokens * runner->model->hidden_channels != x.size()) {
        std::fprintf(stderr, "invalid x size %zu for tokens=%u hidden_channels=%u\n",
                     x.size(),
                     tokens,
                     runner->model->hidden_channels);
        return 1;
    }
    if ((size_t) runner->model->inter_channels * tokens != expected_m.size() ||
        expected_logs.size() != expected_m.size()) {
        std::fprintf(stderr, "invalid expected projection sizes: m=%zu logs=%zu tokens=%u inter_channels=%u\n",
                     expected_m.size(),
                     expected_logs.size(),
                     tokens,
    ...
    runner->project_text_encoder(x.data(), x_mask.data(), tokens, actual_m, actual_logs);
